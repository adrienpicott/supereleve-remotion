...
// Orchestre les scènes : IntroSerie → BossIntro → Hook → Question → Countdown → Reveal → FunFact → CTA
// ════════════════════════════════════════════════════════════════════

import { AbsoluteFill, Sequence, Audio, staticFile, Img, useCurrentFrame, useVideoConfig, spring, interpolate } from 'remotion';
import { VideoConfig, getTheme, TOUR_URL, CARTE_URL, HOOKS, HOOKS_BOSS, LETTERS, SFX_URLS } from './data/shared';
...
export function calculateDuration(cfg: VideoConfig): number {
  ...
  total += cfg.questionDuration; // question display
  total += cfg.countdownDuration; // countdown timer
...
export const TikTokVideo: React.FC<{ config: VideoConfig }> = ({ config }) => {
  ...
  // ── Calculer les offsets de chaque scène (en frames) ──
  let offset = 0;
  ...
  return (
    <AbsoluteFill>
      {/* Background — persiste sur toute la vidéo */}
      ...
      {getScene('hook') && (
        <Sequence from={getScene('hook')!.start} durationInFrames={getScene('hook')!.duration}>
          <HookScene
            hook={config.bossMode && config.personnage ? `${config.personnage.nom} ${bossHook}` : hook}
            config={config} th={th}
          />
          {config.sfxEnabled && (
            <Audio src={SFX_URLS.clicBouton} volume={0.5} />
          )}
        </Sequence>
      )}
      ...
      {getScene('question') && (
        <Sequence from={getScene('question')!.start} durationInFrames={getScene('question')!.duration}>
          ...
          {config.sfxEnabled && (
            <Audio src={SFX_URLS.notification} volume={0.4} />
          )}
        </Sequence>
      )}
      ...
      {getScene('countdown') && (
        <Sequence from={getScene('countdown')!.start} durationInFrames={getScene('countdown')!.duration}>
          ...
          {/* Tick SFX les 3 dernières secondes */}
          {config.sfxEnabled && config.countdownDuration >= 3 && (
            <Audio
              src={SFX_URLS.clicBouton}
              volume={0.3}
              startFrom={Math.max(0, (config.countdownDuration - 3) * FPS)}
            />
          )}
        </Sequence>
      )}
      ...
      {getScene('reveal') && (
        <Sequence from={getScene('reveal')!.start} durationInFrames={getScene('reveal')!.duration}>
          ...
          {config.sfxEnabled && (
            <>
              <Audio src={SFX_URLS.bonneReponse} volume={0.6} />
              <Sequence from={9}>
                <Audio src={SFX_URLS.mauvaiseReponse} volume={0.3} />
              </Sequence>
              {config.lootEnabled && (
                <Sequence from={15}>
                  <Audio src={SFX_URLS.gainXp} volume={0.4} />
                </Sequence>
              )}
            </>
          )}
